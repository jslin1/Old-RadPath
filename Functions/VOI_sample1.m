...
% file = load_nii([ prefix '10_HistMatch/HistMatch_' ptno_Descrip_UID '.nii.gz' ]);
...
cf_orig = file.img(unique(cf_sample)); % Original Rez
...
cf_high = file.img(cf_sample); % High Rez
...
sph_orig = file.img(unique(sph_sample)); % Original Rez
...
sph_high = file.img(sph_sample); % High Rez
